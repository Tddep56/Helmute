...
AudioAnalyzeRMS          rmsInput;        // For monitoring input levels
AudioAnalyzeRMS          rmsNoise;        // For monitoring noise levels
...
AudioConnection          patchCord5(i2s_quadIn, 1, ampLB, 0); // LB is error mic (verified)
...
float baselineLevel = 0;  // To store baseline noise level
...
void setup() {
  ...
  sgtl5000.lineInLevel(15);  // Most sensitive setting (verified working)
  ...
  ampNoise.gain(0.8);  // Increased for better SNR
  ampLB.gain(15);      // Adjusted based on known input levels
...
void finalizeAndSave() {
  ...
  if (dataFile) {
    for (int i = 0; i < 256; i++) {
      dataFile.println(avgTransferFunction[i], 6);  // 6 decimal precision
    }
    dataFile.close();
    Serial.println("Transfer function saved to SD card");
    
    // Print values around our frequency of interest
    Serial.println("\nSample transfer function values:");
    int targetBin = (516 * 1024) / 44100;  // Bin for 516 Hz
    
    for (int i = max(0, targetBin - 2); i <= min(255, targetBin + 2); i++) {
      float freq = (i * 44100.0) / 1024.0;
      Serial.print("Bin ");
      Serial.print(i);
      Serial.print(" (");
      Serial.print(freq, 1);
  ...
  Serial.print("Final audio memory usage: ");
