...
void setup() {
  ...
  // Bandpass filter (200-800 Hz) Gotta start somewhere
  bandpass.setHighpass(0, 200, 0.707);
  bandpass.setLowpass(1, 800, 0.707);
  
  // Configure amplifiers, can work on later but right now good enough
  ampNoise.gain(0.8);  
  ampLB.gain(15);      
  ...
  // Gradually ramp up noise to avoid LOUD NOISES
...
void finalizeAndSave() {
  noise.amplitude(0);        // Turn off noise, DO NOT DELETE FOR SANITY SAKE
